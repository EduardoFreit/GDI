...
BEGIN
    INSERT INTO Ic(codigo, data_inic, tema, cpf_aluno) VALUES (v_codigo, v_data_inic, v_tema, v_cpf_aluno);
    COMMIT;
END insereIC;
/
...
-- BEGIN
--     insereIC(05,to_date('19/02/2018', 'dd/mm/yyyy'),'IoT',3333);
-- END;
...
BEGIN
    ...
    IF v_codigo != -1 THEN
        dbms_output.put_line('TESTE QUESTÕES 49 e 65 ---------> O codigo do curso do aluno com cpf ' || v_cpf || ' é ' || v_codigo);
    END IF;
END;
/
...
-- 69
CREATE OR REPLACE PROCEDURE nomeALuno(v_cpf IN aluno.cpf_aluno%TYPE) AS
v_nome ALUNO.CPF_ALUNO%TYPE;
BEGIN
    SELECT A.nome INTO v_nome FROM aluno A WHERE A.cpf_aluno = v_cpf;
...
-- 76,82
CREATE OR REPLACE TRIGGER red_investimento1
BEFORE UPDATE ON Projeto
FOR EACH ROW
WHEN (NEW.investimento < 100)
BEGIN
    RAISE_APPLICATION_ERROR(-20011,'Investimento muito pequeno');
END;
/

--74,75,80
CREATE OR REPLACE TRIGGER red_investimento2
AFTER UPDATE OF Investimento ON Projeto
FOR EACH ROW BEGIN
    IF(:OLD.Investimento > :NEW.Investimento) THEN
        RAISE_APPLICATION_ERROR (-200111,'Reducao do investimento nao permitida');
    END IF;
END;
/

-- 73 Não deixa deletar nenhum curso, sem condições
...
-- 77
CREATE OR REPLACE TRIGGER Demite 
...
BEGIN
	...
	IF dia != '1' THEN
        RAISE_APPLICATION_ERROR(-20017,'Demissoes sao permitadas apenas no inicio do mes');
    END IF;
END Demite;
/

-- 78,82
CREATE OR REPLACE TRIGGER verifica_Investimento
BEFORE INSERT OR UPDATE ON Projeto
REFERENCING NEW AS NEW OLD AS OLD
FOR EACH ROW DECLARE
BEGIN
    IF :NEW.Investimento > 90000 THEN
        RAISE_APPLICATION_ERROR (-20202,'Valor excessivo de investimento');
    END IF;
END;
/

--79,83
CREATE OR REPLACE TRIGGER deleta_Investimento
BEFORE DELETE ON Projeto
REFERENCING NEW AS NEW OLD AS OLD
FOR EACH ROW DECLARE
BEGIN
    IF :OLD.Investimento > 5000 THEN
        RAISE_APPLICATION_ERROR (-20202,'Investimento muito baixo para ser deletado');
    END IF;
END;
/

-- 81
CREATE OR REPLACE TRIGGER max_alunos
BEFORE INSERT ON Aula	
DECLARE a_count number;
BEGIN
    SELECT COUNT(CPF_aluno) INTO a_count from Aula;
    IF a_count > 40 THEN
    RAISE_APPLICATION_ERROR (-20202, 'numero maximo de alunos por sala atingido');
    END IF; 
END;
/

-- 84
CREATE OR REPLACE TRIGGER insereEmail
AFTER INSERT ON professor
FOR EACH ROW
DECLARE
BEGIN
    INSERT INTO email VALUES(:NEW.Nome||'@cin.ufpe.br', :New.cpf_professor);
END insereEmail;
/

-- 86 Se o professor for excluido, seus projetos tambem serao
CREATE OR REPLACE TRIGGER delete_prof
AFTER DELETE ON professor
FOR EACH ROW
BEGIN
    IF(:OLD.cpf_professor IS NOT NULL) THEN
        DELETE FROM projeto WHERE projeto.cpf_professor = :OLD.cpf_professor;
    END IF;
END;    
/

-- Consulta que usa uma função ja existente
-- 87
...
--91
CREATE OR REPLACE TRIGGER inseP
INSTEAD OF UPDATE ON P_NomeEmail
FOR EACH ROW
DECLARE
BEGIN

    INSERT INTO P_NomeEmail VALUES (:new.nome, :new.email);

END inseP;
/    

--88 e 89 usa um registro como parametro e retorno de uma função
CREATE OR REPLACE FUNCTION prof_proj (proj projeto%ROWTYPE) RETURN professor%ROWTYPE AS
v_prof professor%ROWTYPE;

BEGIN
    SELECT P.cpf_professor, P.data_nasc, P.nome INTO v_prof.cpf_professor,v_prof.data_nasc,v_prof.nome 
    FROM professor P
    WHERE proj.cpf_professor = P.cpf_professor;
